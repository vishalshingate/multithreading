...
### Question: How do you do code review of juniors?
"As a Senior Engineer, I view code reviews for juniors not as an auditing tool, but as my most effective mentorship mechanism. My approach focuses on psychological safety and long-term growth.
1. **Automate the Trivials:** First, I ensure our CI/CD pipeline has strict linters, SonarQube, and formatting checks. I don't want to spend time commenting on trailing spaces or naming conventions; I want to review logic and architecture.
2. **Review the 'Why' First:** Before looking at the code, I read the PR description and tests. If the tests don't reflect the business requirement, the implementation details don't matter yet. 
3. **Collaborative Tone:** I phrase feedback as questions rather than commands. Instead of saying *"This is wrong, use a Map,"* I write, *"What do you think about using a Map here? It might reduce the lookup time to O(1) as this list grows."* This encourages them to think critically rather than just blindly following orders.
4. **Highlight the Good:** Code reviews shouldn't only be negative. When a junior writes a particularly clean function or cleverly handles an edge case, I explicitly call it out (*"Great use of the Strategy pattern here!"*).
5. **Know When to Talk:** If a PR racks up more than 5-6 back-and-forth comments, I pause the async review and jump on a 15-minute screen share or pair-programming session. It breaks down frustration and usually solves the architectural misunderstanding much faster."
